...
const squareIsBlackOrEmpty = function(board, obj) {
  var el = board[obj.row][obj.col];
  if (el === '1')
    return true;
  if (!isUpperCase(el))
    return true;
...
const squareIsWhiteOrEmpty = function(board, obj) {
  var el = board[obj.row][obj.col];
  if (el === '1')
    return true;
  if (isUpperCase(el))
...
var fastGetAvailableSquares = function(board,piece,whiteKingsideCastleAvailable,whiteQueensideCastleAvailable,
  ...
  console.log("fastGetAvailableSquares: row:", row, "col:", col, "piece:", piece, "board:",  board);
  var as = [];
  var color = undefined;
  if (piece == '1') {
    return as;
  ...
  if (color === 'w') {
    return as.filter((obj) => {
      return squareIsBlackOrEmpty(board,obj)
    })
  } else {
    return as.filter((obj) => {
      return squareIsWhiteOrEmpty(board,obj)
    });
  }
};
