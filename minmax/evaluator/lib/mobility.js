    ...
    for (var row = 0; row <= 7; row++) {
        for (var col = 0; col <= 7; col++) {
            ...
            if (piece !== '1') {
                ...
                var squares = fen.getAvailableSquares({histitem:histitem,row:row,col:col,enpassantsquare:fen.enpassantsquare});
                if (color == 'black')     
                    black += squares.availableSquares.length;
                else  
                    white += squares.availableSquares.length;
            }
        }
    }
