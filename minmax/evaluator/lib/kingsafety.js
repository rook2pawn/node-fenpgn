...
var pawnshield = function (b, kingobj) {
  var getUpperLeft = function (row, col) {
    if (row <= 6 && col >= 1) return b[row + 1][col - 1];
    return undefined;
  };
  var getUpperMid = function (row, col) {
    if (row <= 6 && col <= 6) return b[row + 1][col];
    return undefined;
  };
  var getUpperRight = function (row, col) {
    if (row <= 6 && col <= 6) return b[row + 1][col + 1];
    return undefined;
  };
  var getLeft = function (row, col) {
    if (col >= 1) return b[row][col - 1];
    return undefined;
  };
  var getRight = function (row, col) {
    if (col <= 6) return b[row][col + 1];
    return undefined;
  };
  var getLowerLeft = function (row, col) {
    if (row >= 1 && col >= 1) return b[row - 1][col - 1];
    return undefined;
  };
  var getLowerMid = function (row, col) {
    if (row >= 1 && col >= 1) return b[row - 1][col];
    return undefined;
  };
  var getLowerRight = function (row, col) {
    if (row >= 1 && col <= 6) return b[row - 1][col + 1];
    return undefined;
  };
  ...
  if (kingobj.king == "K") {
    ...
    return {
      shield1: shield1,
      shield2: shield2,
      shieldLevels: shieldLevels(shield1, shield2),
    };
...
var difference = function (obj1, obj2) {
  ...
  diffset.pawnshield.white =
    obj1.pawnshield.white.shieldLevels - obj2.pawnshield.white.shieldLevels;
  diffset.pawnshield.black =
    obj1.pawnshield.black.shieldLevels - obj2.pawnshield.black.shieldLevels;
  return diffset;
};
